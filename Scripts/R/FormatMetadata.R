...
Rou_meta_full <- data.frame(read_delim("~/Documents/PhD/Gut-microbiome-immunotherapy/Metadata/Rou_meta.tsv", 
                        "\t", escape_double = FALSE, trim_ws = TRUE),
                        stringsAsFactors = FALSE)
...
Rou_sampInfo = data.frame(read_delim("~/Documents/PhD/Gut-microbiome-immunotherapy/Metadata/PRJEB22863.txt", 
                        "\t", escape_double = FALSE, trim_ws = TRUE),
                        stringsAsFactors = FALSE)
...
Gop_meta = cbind(Gop_meta,rep("MM",nrow(Gop_meta)))
colnames(Gop_meta) = c("Study_patient_id","Response","Treatment","Study","Cancer_type")
...
Mat_meta = cbind(Mat_meta,rep("MM",nrow(Mat_meta)))
Mat_meta$Response = gsub("NonResponder","NR",Mat_meta$Response)
Mat_meta$Response = gsub("Responder","R",Mat_meta$Response)
colnames(Mat_meta) = c("Study_patient_id","Response","Treatment","Study","Cancer_type")
...
Fra_meta = cbind(Fra_meta,rep("MM",nrow(Fra_meta)))
colnames(Fra_meta) = c("Study_patient_id","Response","Treatment","Study","Cancer_type")
...
# Rou_meta
Rou_meta = Rou_meta_full[,c("Title","Response")]
Rou_meta = cbind(Rou_meta,rep("Anti-PD1",nrow(Rou_meta)), rep("Routy_et_al", nrow(Rou_meta)))
Rou_meta = cbind(Rou_meta,Rou_meta_full$host.disease.status)
# Remove T1 samples
Rou_meta = Rou_meta[grepl("T0",Rou_meta[,1]),] 
colnames(Rou_meta) = c("Study_patient_id","Response","Treatment","Study","Cancer_type")
Rou_meta$Response = gsub("Partial response","R",Rou_meta$Response)
Rou_meta$Response = gsub("Stable","NR",Rou_meta$Response)
Rou_meta$Response = gsub("Progression","NR",Rou_meta$Response)
Rou_meta$Response = gsub("Dead","NR",Rou_meta$Response)

# Combine all metadata
clin = rbind(Gop_meta,Mat_meta,Fra_meta,Rou_meta)
rownames(clin) = NULL
...
for (i in 1:nrow(clin)) {
  ...
  if(clin$Study[i] == "Routy_et_al"){
    Patient_id[i] = paste("p",sprintf("%03d",i),"_Rou_",clin$Response[i],sep = "")
  }
}
...
# Remove T1 samples from Routy et al data
Rou_sampInfo = Rou_sampInfo[grepl("T0",Rou_sampInfo$sample_alias),]
...
for (i in 1:nrow(clin)) {
  ...
  if(clin$Study[i] == "Routy_et_al"){
    Sample_id[i] = Rou_sampInfo$run_accession[which(clin$Study_patient_id[i] == Rou_sampInfo$sample_alias)]
  }
}
